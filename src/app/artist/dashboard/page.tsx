...
export default async function ArtistDashboard() {
  ...
  const plainArtist = {
    ...
    studioName: artistRow.studio_name ?? "",
    ...
    certifiedFrom: artistRow.certified_from ?? "",
    travelRadiusKm: artistRow.travel_radius_km ?? 0,
    serviceMode: (artistRow.service_mode ?? "studio") as "studio" | "client" | "both",
    artistType: (artistRow.artist_type ?? "solo") as "solo" | "team",
    maxBookingsPerDay: artistRow.max_bookings_per_day ?? 3,
    cosmeticBrands: artistRow.cosmetic_brands ?? "",
    outstationAvailable: artistRow.outstation_available ?? false,
    outstationConditions: artistRow.outstation_conditions ?? "",
    acneExperience: artistRow.acne_experience ?? false,
    acneExperienceDetails: artistRow.acne_experience_details ?? "",
    paymentStructure: artistRow.payment_structure ?? "",
    paymentModes: artistRow.payment_modes ?? "",
    invoiceAvailable: artistRow.invoice_available ?? false,
    paymentNotes: artistRow.payment_notes ?? "",
